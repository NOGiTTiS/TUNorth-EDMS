...
export default function DashboardLayout({
  ...
  const menuItems = [
    ...
    ...(user.role === 'admin_central' ? [{ name: 'ตั้งค่าระบบ', icon: Settings, href: '/dashboard/settings' }] : []),
  ];
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <aside className="w-64 bg-white dark:bg-slate-800 border-r shadow-sm hidden md:flex flex-col">
        ...
        <div className="p-6 border-b flex flex-col items-center gap-3 text-center">
          {settings.logo_url ? (
              <img src={getImageUrl(settings.logo_url)} alt="Logo" className="h-16 object-contain drop-shadow-sm" />
          ) : (
              ...
              <Link key={item.href} href={item.href}>
                <span 
                  className={cn(
                    ...
                    !isActive && "text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-700"
                  )}
                  // 3. จัดการสี Focus (Active) ให้ตรงกับ Theme Setting
                  style={isActive ? { 
                    backgroundColor: `${settings.theme_main_color}1A`, // เติม 1A คือ Opacity 10%
                    color: settings.theme_main_color,
                    borderRight: `3px solid ${settings.theme_main_color}`
                  } : {}}
                >
        ...
        <div className="p-4 border-t bg-slate-50/50">
          <div className="flex items-center gap-3 mb-4 px-2">
            ...
            <div className="overflow-hidden">
              <p className="text-sm font-medium truncate text-slate-800">{user.full_name}</p>
      ...
      <main className="flex-1 overflow-y-auto bg-theme-grad">
        <header className="h-16 bg-white/80 backdrop-blur-md border-b flex items-center justify-between px-6 md:hidden">
            <span className="font-bold text-theme-main">{settings.system_name}</span>

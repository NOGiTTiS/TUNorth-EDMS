  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("สมัครสมาชิกสำเร็จ!", {
        description: "กรุณาเข้าสู่ระบบด้วยบัญชีที่สร้าง",
      })
      router.push("/")
    } catch (error: any) {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error ||
          "ไม่พบ Chat ID ล่าสุด (ลองกด Start ที่บอทหรือยัง?)",
      )
    } finally {
    ...
    <div className="min-h-screen flex flex-col items-center justify-start sm:justify-center bg-theme-grad p-4 py-8 sm:py-12">
      <Card className="w-full max-w-md shadow-2xl border-t-4 border-theme-main bg-white/95 backdrop-blur-sm overflow-hidden">
        <CardHeader className="space-y-1 text-center">
          <div className="flex justify-center mb-4">
            {settings.logo_url ? (
              ...
              <div className="w-14 h-14 sm:w-16 sm:h-16 bg-theme-main rounded-full flex items-center justify-center text-white text-2xl sm:text-3xl font-bold shadow-lg">
                {settings.system_name?.charAt(0) || "T"}
              </div>
            )}
          </div>
          <CardTitle className="text-xl sm:text-2xl font-bold text-theme-main">
            สร้างบัญชีใหม่
          </CardTitle>
        ...
        <form onSubmit={handleRegister}>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-9 bg-yellow-50/50"
          ...
          <CardFooter className="flex flex-col gap-4 pb-6">
            <Button
              type="submit"
              className="w-full bg-theme-main text-white hover:brightness-90 transition-all shadow-md h-12 mt-6 text-md font-semibold"
              disabled={isLoading}
            >
              {isLoading ? (
      ...
      <Dialog open={isHelpOpen} onOpenChange={setIsHelpOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl text-center pb-2">
            ...
            <DialogDescription asChild>
              <div className="text-slate-600 space-y-4 pt-2">
                <div className="space-y-2">
                  <p>
                    1. คลิกปุ่มด้านล่างเพื่อเปิดแชทกับบอทของเรา แล้วกด{" "}
                    <b>"Start"</b> หรือส่งข้อความอะไรก็ได้ 1 ข้อความ
                  </p>
                  <Button
                    variant="outline"
                    ...
                    onClick={() =>
                      window.open(
                        `https://t.me/${settings.telegram_username?.replace("@", "")}`,
                        "_blank",
                      )
                    }
                  >
                ...
                <div className="space-y-2 pt-2">
                  <p>
                    2. กลับมาที่หน้านี้ แล้วคลิกปุ่ม <b>"ดึง Chat ID ล่าสุด"</b>
                  </p>
                </div>
              </div>
            </DialogDescription>
